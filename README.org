...
Additionally, ~meow-tree-sitter-node~ provides a tree-sitter powered drop-in replacement for ~meow-block~. It will automatically fallback in non-tree-sitter buffers, and works with any major mode without the need for queries (since it simply parses node boundaries) ⚡.
